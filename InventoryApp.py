    ...
    def report(self):
        ...
        def showReport(month = calendar.month_name[date] ,type = "all" ):
            ...
            for r in range(startingValue, endingValue):                
                ...
                tableEntry = Button(itemContainer,background=backgroundColor, width = 17 ,height=2,text=str(r + 1),font="-family {Segoe UI} -size 10 -weight bold", state='disabled', justify='center' ,foreground="Black")
                #tableEntry= Entry(itemContainer ,width=16, fg='Black',border= 2,state='disabled',justify= 'center', textvariable=element, font="-family {Segoe UI} -size 12 -weight bold ",foreground ="Black",background=backgroundColor)
                tableEntry.grid(column=0 , row = 0)
            ...
            ## test amount  
            sold = str(sum([allreports[i][6] for i in range(len(allreports)) if allreports[i][2] == "Sales"]))
            purchased = str(sum([allreports[i][6] for i in range(len(allreports)) if allreports[i][2] == "Order"]))
